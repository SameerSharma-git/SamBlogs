...
const Navbar = () => {
    ...
    const [triggerLoadingBar, setTriggerLoadingBar] = useState(0)
    ...
    const handleLinkClick = (e) => {
        ...
        if (target) {
            setTriggerLoadingBar(triggerLoadingBar + 1)
        }
    }
    ...
    return (
        <>
            {/* Navbar component */}
            <nav onClick={handleLinkClick} className={`bg-navbar-bg bg-opacity-50 backdrop-filter backdrop-blur-lg p-4  shadow-lg rounded-b-lg ${!(pathname.startsWith('/blogpost') || pathname.startsWith('/dashboard') || pathname.startsWith('/contact')) && 'sticky'} top-0 z-20`}>
                <LoadingBar
                    color="#2563EB"
                    progress={progress}
