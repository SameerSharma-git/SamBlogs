...
export async function GET() {
  ...
  `,
        ...
        async function inititalise(userId, userName, blogs) {
            let blogsId = []
            blogs.forEach(blog => {
                blog.userId = userId
            ...
            await PostSchema.insertMany(blogs)

            await ((await PostSchema.find({ userId: userId })).forEach(blog => {
        ...
        await (users.forEach((user, index) => {
            inititalise(user._id, user.name, blogArray[index])
        }))
